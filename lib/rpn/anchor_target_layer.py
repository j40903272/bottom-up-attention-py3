...
class AnchorTargetLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        if cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0:
            ...
            positive_weights = (old_div(cfg.TRAIN.RPN_POSITIVE_WEIGHT,
                                np.sum(labels == 1)))
            negative_weights = (old_div((1.0 - cfg.TRAIN.RPN_POSITIVE_WEIGHT),
                                np.sum(labels == 0)))
        ...
        if DEBUG:
            ...
            means = old_div(self._sums, self._counts)
            stds = np.sqrt(old_div(self._squared_sums, self._counts) - means ** 2)
            print('means:')
            print(means)
            print('stdevs:')
            print(stds)
        ...
        if DEBUG:
            ...
            self._count += 1
            print('rpn: num_positive avg', old_div(self._fg_sum, self._count))
            print('rpn: num_negative avg', old_div(self._bg_sum, self._count))
